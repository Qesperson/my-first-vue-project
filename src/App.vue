<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div
              v-show="hintList.length"
              class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap"
            >
              <span
                v-for="hint in getForHints()"
                :key="hint"
                @click="addTickerFromHint(hint)"
                class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer"
              >
                {{ hint }}
              </span>
            </div>
            <div v-show="showAlert" class="text-sm text-red-600">
      ...
      <section v-show="sel" class="relative">
        <h3 class="text-lg leading-6 font-medium text-gray-900 my-8">
          {{ sel?.name }} - USD
        </h3>
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div
            v-for="(bar, indx) in toNormalizeGraph()"
            :key="indx"
            :style="{ height: `${bar}%` }"
...
export default {
  ...
  methods: {
    ...
    onAdd() {
      ...
      if (!this.tickersNames.includes(this.ticker)) {
    ...
    getForHints() {
      // this.showAlert = false
      return (this.hintList = this.coinList
        .filter((c) =>
          this.ticker
            ? c.toLowerCase().includes(this.ticker.toLowerCase())
            : false
        )
        .slice(0, 4))
    },
